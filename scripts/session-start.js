...
const rules = {
    ...
    ultra: "COMPRESS ULTRA ACTIVE\n- Use MetaGlyph: ∈ (in/contains), → (leads to/causes), ∀ (all), ∃ (exists), ∴ (therefore), ! (critical).\n- Chain of Draft (CoD): minimal thinking steps, immediate action output.\n- Abbreviate prose (DB, auth, config, req, res, fn, impl, ctx, err, msg, val, bool, pkg).\n- Strip conjunctions. One word when one word enough.\n- Goal: Maximal density."
};
