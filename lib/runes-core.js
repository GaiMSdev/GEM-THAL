...
const VALID_MODES = ['lite', 'full', 'ultra', 'hybrid', 'off'];
const TMP_DIRS = [
...
function getStatsReport() {
  ...
  const COMPRESSION = { lite: 0.35, full: 0.75, ultra: 0.88, hybrid: 0.92 };
...
function getPromptInstructions(mode) {
  ...
  if (mode === 'ultra') return "COMPRESS ULTRA: Use MetaGlyphs ONLY. Extreme brevity. No prose. <R> packets.";
  if (mode === 'hybrid') return "COMPRESS HYBRID: Combined technique. Broken-English + MetaGlyphs. Use 'Me' + subject/verb fragment. Dense logic via glyphs (⌬, →, ⚙). Maximize token-savings while maintaining linguistic anchor.";
